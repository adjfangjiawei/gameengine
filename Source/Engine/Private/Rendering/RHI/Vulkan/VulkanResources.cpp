...
namespace Engine {
    namespace RHI {
        ...
        bool VulkanBuffer::CreateBuffer() {
            ...
            // 根据缓冲区用途设置使用标志
            if (Desc.Flags & ERHIResourceFlags::AllowUnorderedAccess) {
                bufferInfo.usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
            }
            if (Desc.Stride > 0) {
                bufferInfo.usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
            }
            if (Desc.Flags & ERHIResourceFlags::AllowRenderTarget) {
        ...
        bool VulkanBuffer::AllocateMemory() {
            ...
            if (Desc.Access & ERHIAccessFlags::CPURead) {
                properties |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
            }
            if (Desc.Access & ERHIAccessFlags::CPUWrite) {
                properties |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
            }
            if (!(Desc.Access &
                  (ERHIAccessFlags::CPURead | ERHIAccessFlags::CPUWrite))) {
        ...
        void* VulkanBuffer::Map(uint32 subresource) {
            if (MappedData) {
                return MappedData;
            }
        ...
        void VulkanBuffer::Unmap(uint32 subresource) {
            if (MappedData) {
        ...
        bool VulkanTexture::CreateImage() {
            ...
            if (Desc.Flags & ERHIResourceFlags::AllowDepthStencil) {
                imageInfo.usage |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
            }
            if (!(Desc.Flags & ERHIResourceFlags::DenyShaderResource)) {
        ...
        bool VulkanTexture::CreateDefaultView() {
        ...
        uint64 VulkanTexture::GetSize() const {
        ...
        VulkanShader::~VulkanShader() {
            if (ShaderModule != VK_NULL_HANDLE) {
                vkDestroyShaderModule(
                    Device->GetHandle(), ShaderModule, nullptr);
            }
        }

        bool VulkanShader::CreateShaderModule(const void* shaderData,
                                              size_t dataSize) {
            ...
            return vkCreateShaderModule(Device->GetHandle(),
                                        &createInfo,
                                        nullptr,
        ...
        bool VulkanRenderTargetView::CreateImageView() {
        ...
        bool VulkanDepthStencilView::CreateImageView() {
            auto texture = static_cast<VulkanTexture*>(Resource);
            if (!texture) {
                return false;
            }
        ...
        bool VulkanShaderResourceView::CreateView() {
            if (Resource->GetResourceDimension() ==
                ...
                return vkCreateBufferView(Device->GetHandle(),
                                          &viewInfo,
                                          nullptr,
                ...
                return vkCreateImageView(Device->GetHandle(),
                                         &viewInfo,
                                         nullptr,
        ...
        bool VulkanUnorderedAccessView::CreateView() {
            if (Resource->GetResourceDimension() ==
                ERHIResourceDimension::Buffer) {
                auto buffer = static_cast<VulkanBuffer*>(Resource);
                if (!buffer) {
