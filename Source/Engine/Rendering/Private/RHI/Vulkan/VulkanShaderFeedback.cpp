...
namespace Engine {
    namespace RHI {
        ...
        VulkanShaderFeedbackBuffer::VulkanShaderFeedbackBuffer(
            VulkanDevice* InDevice, const ShaderFeedbackDesc& InDesc)
            : Device(InDevice),
              Desc(InDesc),
        ...
        void VulkanPredicate::SetValue(const PredicateDesc& value) {
            ...
            switch (value.Type) {
                case EPredicateType::Binary:
                    predicateValue = value.Binary.Value ? VK_TRUE : VK_FALSE;
                    break;
                case EPredicateType::Numeric:
                    predicateValue = value.Numeric.Value;
                    break;
                case EPredicateType::CompareValue:
                    // 在这种情况下，我们需要根据比较操作符和引用值来设置谓词值
                    predicateValue =
                        (value.Numeric.Value >= value.Numeric.Reference)
                            ? VK_TRUE
                            : VK_FALSE;
                    break;
            }
